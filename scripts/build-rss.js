...
const feed = new Feed({
  ...
  copyright: `All rights reserved ${new Date().getFullYear()}, Jamshidbek Tashpulatov`,
  feedLinks: {
  ...
  author: {
    name: 'Jamshidbek Tashpulatov',
    link: 'https://twitter.com/@heyjamshid',
  },
})
...
getAllPosts().forEach(({ link, module: { meta, default: Content } }) => {
  ...
  const postText = `<p><em>(The post <a href="${siteUrl + link}">${
    meta.title
  }</a> appeared first on <a href="${siteUrl}">Jamshidbek Tashpulatov Personal Blog</a>.)</em></p>`
  feed.addItem({
    title: meta.title,
    id: meta.title,
    link,
